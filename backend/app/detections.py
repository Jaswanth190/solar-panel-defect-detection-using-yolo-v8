...
def _serialize(doc: dict) -> dict:
    """Convert ObjectId fields to strings and handle datetime objects."""
    if doc is None:
        return None
    doc["_id"] = str(doc["_id"])
    if "timestamp" in doc and isinstance(doc["timestamp"], datetime):
        # Force UTC 'Z' suffix for frontend parsing consistency
        doc["timestamp"] = doc["timestamp"].replace(tzinfo=timezone.utc).isoformat().replace("+00:00", "Z")
    return doc
...
async def save_detection(clerk_user_id: str, entry: dict) -> dict:
    ...
    """
    # Use UTC for "perfectly dynamic" sync as requested
    doc = {
