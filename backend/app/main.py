...
from app.sync import sync_user, get_all_users, update_user_access, get_user
...
@app.delete("/admin/users/{clerk_id}")
async def remove_user(clerk_id: str):
    """
    Delete machine status or user record from DB.
    """
    from app.sync import delete_user
    success = await delete_user(clerk_id)
    if not success:
        raise HTTPException(status_code=404, detail="User not found")
    return {"status": "deleted", "clerk_id": clerk_id}

@app.get("/admin/me/{clerk_id}")
async def get_my_role(clerk_id: str):
    """
    Get user role from MongoDB.
    """
    user = await get_user(clerk_id)
    if not user:
        return {"role": "user", "hasAccess": False}
    return {"role": user.get("role", "user"), "hasAccess": user.get("hasAccess", False)}

@app.get("/admin/fix-roles/{email}")
async def fix_admin_roles(email: str):
    """
    Ensure ONLY the user with the specified email has the 'admin' role. All others are reset to 'user'.
    """
    from app.db import users_collection
    # First downgrade everyone
    await users_collection.update_many(
        {}, 
        {"$set": {"role": "user", "hasAccess": False}}
    )
    # Then upgrade ONLY the requested email
    result = await users_collection.update_one(
        {"email": email}, 
        {"$set": {"role": "admin", "hasAccess": True}}
    )
    return {"status": "fixed", "admin_email": email, "found": result.modified_count > 0}
...
async def sync_clerk_user(user_data: dict):
    ...
    """
    db_user = await sync_user(user_data)
    if db_user:
