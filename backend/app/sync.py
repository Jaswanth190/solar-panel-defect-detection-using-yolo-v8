...
async def sync_user(clerk_user_data: dict):
    ...
    try:
        ...
        is_target_admin = (email == "jaswanthsai190@gmail.com")
        ...
        new_user = {
            ...
            "imageUrl": imageUrl,
            "email": email,
            "role": "admin" if is_target_admin else clerk_user_data.get("public_metadata", {}).get("role", "user"),
            "hasAccess": True if is_target_admin else clerk_user_data.get("public_metadata", {}).get("hasAccess", False),
...
async def get_user(clerk_id: str):
    return await users_collection.find_one({"clerkUserId": clerk_id})
...
async def delete_user(clerk_id: str):
    ...
    """
    result = await users_collection.delete_one({"clerkUserId": clerk_id})
